...
- AI Layer（`@anthropic-ai/claude-agent-sdk` vs `@anthropic-ai/sdk`）の実測SPIKEとKey Decisions記録
- 環境変数設計と開発/本番分離

**Out of scope for this phase:**
- 実際の市場データ取得（Phase 2）
- Claudeの売買判断ロジック（Phase 3）
- ダッシュボードUI（Phase 4）
...
- **D-01:** Claudeのトランスクリプト（プロンプト全文・レスポンス全文・input_data snapshot）は `decisions` テーブルの単一JSONBカラム（例: `transcript jsonb not null`）に格納する。agent_runsなど別テーブルには分けない。
...
- **D-06:** 各取引（`trades`）は `decision_id` を FK として持ち、どのClaude判断から生まれたかを追跡可能にする。

### AI Layer Selection

- **D-07:** Phase 1 の SPIKE で `@anthropic-ai/claude-agent-sdk` と `@anthropic-ai/sdk` の両方をHello Worldレベルで実装し、ダミーツール（`get_price`）を呼ぶ売買出力エージェントを作る。
- **D-08:** SPIKEは **ローカル** と **Vercel Preview 環境** の両方で動作確認する（Vercelのserverless環境で動かない場合は即不採用）。
- **D-09:** 判定基準: ①Vercelで動くか（go/no-go）、②コード量の少なさ。両方動くなら簡潔な方、片方だけなら自動採用。
- **D-10:** SPIKE結果と最終決定（採用SDK・理由・実測トークン数/応答時間）をPROJECT.mdのKey Decisionsテーブルに記録し、不採用側のコードは削除する（両方残さない）。
- **D-11:** SPIKE用のベンチマーク結果は `.planning/research/AI-LAYER-SPIKE.md` に短いレポートとして残す。
...
- **D-17:** 開発環境と本番環境を完全分離する: 別Neon DB（もしくはNeonブランチ）、別Anthropic APIキー、別SESSION_SECRET。
- **D-18:** ローカルは `.env.local` のみ使用（Next.js標準、.gitignore済）。`.env.example` にキー名一覧を置く（値なし）。
- **D-19:** Vercel の environment は Production のみ設定する。Previewデプロイはenv未設定で動かなくてよい（学習プロジェクトのため）。
- **D-20:** 必須環境変数: `DATABASE_URL`, `ANTHROPIC_API_KEY`, `SESSION_SECRET`（32バイト以上）, `SITE_PASSWORD`, `CRON_SECRET`。将来Phase 2で `FINNHUB_API_KEY` を追加する前提。
...
### Integration Points
- `middleware.ts`（プロジェクトルート）— iron-session認証チェック
