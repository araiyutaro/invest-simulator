...
- [ ] **Phase 3: Agent Pipeline** - Geminiが市場データを参照して売買判断し、トランスクリプトと仮想トレードをDBに保存する
...
**Plans**: 5 plans
  ...
  - [x] 01-04-PLAN.md — Next.js 16 proxy.ts auth gate + /dashboard placeholder + end-to-end verify
...
**Goal**: Geminiが市場データを参照して買い/売り/ホールドを判断し、full transcript と仮想トレードの結果がDBに永続化される
**Depends on**: Phase 2
**Requirements**: AGENT-01, AGENT-02, AGENT-03, AGENT-04, AGENT-05, AGENT-06, AGENT-07, EXEC-01, EXEC-02, EXEC-03, EXEC-04, EXEC-05
**Success Criteria** (what must be TRUE):
  1. `/api/cron/daily-run` を叩くと Gemini が価格・ニュース・ポジションを参照して sell/buy/hold を判断し、`decisions` テーブルにfull transcript（JSONB）・token count・estimated cost が保存される
  2. Geminiの判断に基づいた仮想取引が `trades` テーブルに記録され、`positions` テーブルの保有数・現金残高が更新される
...
**Plans**: 4 plans
  - [x] 03-01-PLAN.md — 共有型定義（zodスキーマ）+ プロンプト組み立て（ニュース圧縮・TA指標）
  - [x] 03-02-PLAN.md — Gemini API呼び出し（responseSchema・リトライ・コスト推定）
  - [x] 03-03-PLAN.md — 仮想売買執行ロジック（残高チェック・FX変換・avgCost計算）
  - [x] 03-04-PLAN.md — daily-run Route Handler + データローダー（オーケストレーター）
...
**Plans**: 4 plans
  - [ ] 04-01-PLAN.md — ダークテーマ・ライブラリインストール・1306.Tホワイトリスト・ページシェル
  - [ ] 04-02-PLAN.md — データレイヤーTDD（型定義・metrics計算・DBクエリ）
  - [ ] 04-03-PLAN.md — チャート・指標グリッド・ポジションテーブル・配分パイチャート
  - [ ] 04-04-PLAN.md — トレードタイムライン・追加読み込みAPI
**UI hint**: yes
...
| 3. Agent Pipeline | 0/4 | Planned | - |
| 4. Dashboard | 0/4 | Planned | - |
