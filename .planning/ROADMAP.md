...
**Plans**: 4 plans
  - [ ] 03-01-PLAN.md — 共有型定義（zodスキーマ）+ プロンプト組み立て（ニュース圧縮・TA指標）
  - [ ] 03-02-PLAN.md — Gemini API呼び出し（responseSchema・リトライ・コスト推定）
  - [ ] 03-03-PLAN.md — 仮想売買執行ロジック（残高チェック・FX変換・avgCost計算）
  - [ ] 03-04-PLAN.md — daily-run Route Handler + データローダー（オーケストレーター）
