...
> **Critical Note on Claude Agent SDK (`@anthropic-ai/claude-agent-sdk` v0.2.101):** This SDK is designed for agentic code-execution tasks (read files, run bash, edit code). It spawns Claude Code as a subprocess. Vercel Hobby plan allows only 45-minute sandboxes and needs `spawnClaudeCodeProcess` customisation. For this project's use case—supplying market data context and receiving a trade decision—the regular `@anthropic-ai/sdk` with tool_use and a hand-rolled agentic loop is architecturally correct and avoids the container overhead entirely. Revisit the Agent SDK if future milestones require multi-step autonomous code execution.
...
**Fallback strategy:** If yahoo-finance2 breaks (Yahoo changes their internal API), fall back to Stooq CSV downloads for JP price history via HTTP GET (no API key, but no fundamentals). Log API errors and skip the affected ticker rather than failing the entire daily job.
...
**Schema key tables:** `trades`, `positions`, `daily_snapshots`, `claude_logs` (stores prompt, response, model, tokens, timestamp), `price_cache` (daily OHLCV per ticker).
...
**Decision:** Use lightweight-charts for all time-series (portfolio curve, price history). Use Recharts only for allocation/distribution charts that have <50 data points.
...
**Recommendation:** Vercel Cron (`vercel.json` `crons` key). Configure `0 7 * * *` (UTC 7am = JST 4pm, after TSE close) or `0 22 * * 1-5` (UTC 10pm = after NYSE close). Hobby plan allows once-per-day — this constraint aligns with the project's 1-day cycle requirement.

> Caveat: Vercel Hobby cron fires "within the configured minute's hour" — not guaranteed to the exact minute. For a daily educational simulator, this is acceptable.
...
**Recommendation:** `iron-session` v8 with Next.js middleware. Password stored in `DASHBOARD_PASSWORD` env var. Session cookie encrypted with a 32+ char `SESSION_SECRET` env var. Login page → sets `isAuthenticated: true` in encrypted cookie → middleware checks cookie on all `/` routes. v8 specifically targets App Router via `getIronSession(cookies(), { ... })`.
...
**Recommendation:** Use `technicalindicators` for pre-computing RSI, MACD, 20/50-day SMA before passing to Claude. Do not compute inline — isolate in a `lib/indicators.ts` utility. This keeps the Claude prompt function pure (input: data → output: indicators) and testable.
...
*Stack research for: AI-driven virtual stock trading simulator*
*Researched: 2026-04-11*
